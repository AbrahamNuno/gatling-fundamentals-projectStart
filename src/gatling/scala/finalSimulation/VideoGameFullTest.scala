...
class VideoGameFullTest extends BaseSimulation {
  ...
  // Other Variables
  val idNumbers = (20 to 1000).iterator
  val rnd = new Random()
  ...
  /*** Custom Feeder ***/
  val custoFeeder = Iterator.continually(Map(
  ...
  def postNewGame() = {
    feed(custoFeeder)
      .exec(http("Post a New Game")
        .post("videogames")
        .body(ElFileBody("NewGameTemplate.json")).asJSON // Template file is located in gatling/resources/bodies
        .check(status.is(200)))
  }
